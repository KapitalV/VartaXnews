...
export type AdSlotPosition = 'ticker_top' | 'article_modal' | 'sidebar' | 'banner_header';

export interface LocalAd {
  id: string;
  clientName: string;
  bannerUrl: string;
  targetUrl?: string; // WhatsApp or website or tel:
  phone?: string;
  slot: AdSlotPosition;
  tickerText?: string; // If displayed in ticker
  isActive: boolean;
  startDate?: string;
  endDate?: string;
  createdAt: string;
