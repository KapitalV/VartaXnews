...
export async function checkMigrationStatus(): Promise<MigrationSummary> {
  ...
  if (configured) {
    ...
    if (supabase) {
      try {
        // Try varta_news_posts first, then posts
        let { count: pCount, error: pErr } = await supabase
          .from('varta_news_posts')
          .select('*', { count: 'exact', head: true });

        if (pErr || typeof pCount !== 'number') {
          const { count: altPCount } = await supabase
            .from('posts')
            .select('*', { count: 'exact', head: true });
          pCount = altPCount;
        }
...
export async function syncLocalDataToSupabase(): Promise<{ 
  ...
  if (!isSupabaseConfigured()) {
    return {
      ...
      syncedTeam: 0,
      syncedVideos: 0,
      error: 'Supabase पर्यावरण वेरिएबल्स (VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY) उपलब्ध नहीं हैं।'
    };
  }
  ...
  try {
    ...
    // 1. Sync Posts to varta_news_posts & posts
    const localPosts = getStoredPosts();
    for (const p of localPosts) {
      ...
      try {
        await supabase.from('varta_news_posts').upsert(payload, { onConflict: 'id' });
      } catch {}
