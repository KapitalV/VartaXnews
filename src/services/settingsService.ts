...
export async function fetchSiteSettings(): Promise<Partial<SiteSettings>> {
  ...
  let logoStored: string | null = null;
  let anshStored: string | null = null;
  try {
    logoStored = localStorage.getItem('varta_channel_logo');
    anshStored = localStorage.getItem('varta_ansh_photo');
  } catch {}
...
export async function updateSetting(key: string, value: any): Promise<void> {
  ...
  if (key === 'channel_logo') {
    safeStorageSet('varta_channel_logo', typeof value === 'string' ? value : JSON.stringify(value));
  } else if (key === 'ansh_photo') {
    safeStorageSet('varta_ansh_photo', typeof value === 'string' ? value : JSON.stringify(value));
  }
}
