...
import { NewsPost, VideoBulletin, TeamMember, PushNotification } from './types';
import { fetchPosts, subscribeToPosts, incrementPostViews, incrementPostLikes, createPost } from './services/postService';
...
export function App() {
  ...
  // Core Supabase Data Models
  const [posts, setPosts] = useState<NewsPost[]>([]);
  const [teamMembers, setTeamMembers] = useState<TeamMember[]>([]);
  const [videos, setVideos] = useState<VideoBulletin[]>([]);
  ...
  // Initial Data Fetch & Supabase Subscriptions
  useEffect(() => {
    let isMounted = true;
    ...
    async function loadData() {
      setIsLoading(true);
      try {
        const [postsData, teamData, videosData] = await Promise.all([
          fetchPosts(),
          fetchTeamMembers(),
          fetchVideos()
        ]);
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] text-[#111827] flex flex-col antialiased">
      ...
      {breakingPosts.length > 0 && activeTab === 'feed' && (
        <BreakingTicker 
