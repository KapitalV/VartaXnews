...
export default function HoroscopeSection({ initialSignId }: HoroscopeSectionProps = {}) {
  const [selectedSignId, setSelectedSignId] = useState<string>(() => {
    if (initialSignId && ZODIAC_SIGNS.some(s => s.id === initialSignId.toLowerCase())) {
      return initialSignId.toLowerCase();
    }
    return 'aries';
  });
  const [sharingSign, setSharingSign] = useState<ZodiacSign | null>(null);
  const [nameInput, setNameInput] = useState<string>('');
  const [discoveredSign, setDiscoveredSign] = useState<ZodiacSign | null>(null);
  
  // Sync if initialSignId changes
  useEffect(() => {
    if (initialSignId && ZODIAC_SIGNS.some(s => s.id === initialSignId.toLowerCase())) {
      setSelectedSignId(initialSignId.toLowerCase());
    }
  }, [initialSignId]);
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="bg-[#111111] border border-amber-500/30 rounded-3xl p-6 sm:p-8 space-y-6 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-white/10">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
            ...
            <button 
              onClick={() => setSharingSign(activeSign)}
              className="bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-extrabold text-xs px-4 py-3 rounded-2xl transition shadow-lg flex items-center gap-2 cursor-pointer active:scale-98"
            >
              <Share2 className="h-4 w-4" />
              <span>✨ राशिफल शेयर करें</span>
            </button>
          </div>
        </div>
      ...
      {/* Dedicated Horoscope Share Modal with Canvas HD Photo Card Generator */}
      <HoroscopeShareModal 
        sign={sharingSign} 
        isOpen={Boolean(sharingSign)} 
        onClose={() => setSharingSign(null)} 
      />
