...
import { safeStorageSet, safeStorageGet, sanitizePostsForStorage } from './utils/safeStorage';

export { safeStorageSet, safeStorageGet, sanitizePostsForStorage };
...
export function getStoredPosts(): NewsPost[] {
  const parsed = safeStorageGet<NewsPost[]>('varta_x_posts', []);
...
export function savePosts(posts: NewsPost[]): void {
  ...
  safeStorageSet('varta_x_posts', JSON.stringify(sanitized));
}

export function getStoredTeam(): TeamMember[] {
  const parsed = safeStorageGet<TeamMember[]>('varta_x_team', []);
  if (Array.isArray(parsed) && parsed.length > 0) {
    // Filter out obsolete legacy test members if any
    const filtered = parsed.filter(m => !m.name.toLowerCase().includes('ramesh') && m.id !== 'team-2');
    return filtered;
  }
...
export function getStoredVideos(): VideoBulletin[] {
  const parsed = safeStorageGet<VideoBulletin[]>('varta_x_videos', []);
  if (Array.isArray(parsed) && parsed.length > 0) {
    return parsed;
  }
